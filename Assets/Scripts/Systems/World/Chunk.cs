...
namespace Zoo.Systems.World
{
    ...
    public class Chunk : MonoBehaviour
    {
        ...
        //tweakables
        public int chunkSize = 25;  //max tiles per side this chunk can handle
        ...
        void DrawTile(QuarterTile t, int terrainTexture, int cliffTexture = -1)
        {

            QuarterTile[] adjacentTiles = world.GetAdjacentQuarterTiles(t);
            ...
            if (pointsAtReference == 3)
            {
                ...
                if ((!t.upperLeft.y.Equals(t.height)) || (!t.lowerRight.y.Equals(t.height)))
                {
                    ...
                    if (cliffTexture > -1)
                    {
                        ...
                        if (adjacentTiles[0] == null)
                        {
                            ...
                            tris[cliffTexture].Add(vertCount + 9);    //2        3
                            tris[cliffTexture].Add(vertCount + 5);    //3        7

                            tris[cliffTexture].Add(vertCount + 4);    //1        0
                            tris[cliffTexture].Add(vertCount + 8);    //3        7 
                            tris[cliffTexture].Add(vertCount + 9);    //4        5
                        }
                        // Do cliff triangles for W face
                        if (adjacentTiles[2] == null)
                        {
                ...
                {
                    Debug.Log($"Skipped mesh drawing for tile {t.TileCoordX},{t.TileCoordZ}!");
                }
                #endregion
            }
            //two points off-spec
        ...
        /// <summary>
        /// Draw all tiles on the chunk.
        /// </summary>
        /// <param name="map"></param>
        /// <param name="selectionTilePass">True if current pass is for the selection tile map.</param>
        public void DrawTiles(QuarterTile[,] map, bool selectionTilePass)
        {
            ...
            for (int x = startX; x < endX; x++)
            {
                for (int z = startZ; z < endZ; z++)
                {
                    if (map[x, z] != null)
                    {
                        QuarterTile t = map[x, z];
                        if (selectionTilePass)
                        {
                            DrawTile(t, t.OverlayTileType);
                        }
                        else
                        {
                            DrawTile(t, t.TerrainType, t.CliffType);
                        }
                    }
                    else
                    {
                        //Debug.Log($"Skipping null tile at {x},{z}, on {world.GetChunk(x/chunkSize,z/chunkSize).name}!");
                    }
                }
            }
            //then commit to the changes
            CommitMeshChanges();
        ...
        void CommitMeshChanges()
        {
            ...
            for (int i = 0; i < mesh.subMeshCount; i++)
            {
                ...
                {
                    //Debug.Log("Setting triangles for submesh #" + i);
        ...
        bool CheckSouthHeight(QuarterTile[] tiles, QuarterTile t)
        {
        ...
        bool CheckWestHeight(QuarterTile[] tiles, QuarterTile t)
        {
        ...
        bool CheckNorthHeight(QuarterTile[] tiles, QuarterTile t)
        {
        ...
        bool CheckEastHeight(QuarterTile[] tiles, QuarterTile t)
        {
